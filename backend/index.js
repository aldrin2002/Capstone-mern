...
// Make sure these are accessible to your route handlers
app.set("connectedUsers", connectedUsers);
app.set("adminSockets", adminSockets);


// Improved broadcastOnlineCustomers function
const broadcastOnlineCustomers = () => {
  const onlineCustomersList = Array.from(onlineCustomers);
  console.log(
    `📡 Broadcasting online customers to ${adminSockets.size} admins:`,
    onlineCustomersList
  );

  // Method 1: Send to each admin socket individually
  ...
  // Method 2: Also broadcast to admin room (backup)
  io.to("admin-room").emit("customer-status-update", {
    customers: onlineCustomersList,
  });

  console.log(`✅ Broadcasted to ${adminSockets.size} admin sockets`);
};
...
io.use((socket, next) => {
  try {
    ...
    if (!authToken && socket.request.headers.cookie) {
      ...
      socket.role = decoded.role || "customer";
      console.log(
        `Socket authenticated via auth object: User ${decoded.userId}`
      );
      return next();
    }
...
io.on("connection", async (socket) => {
  ...
  try {
    ...
    if (user.role === "admin") {
      ...
      // Send immediate status update to newly connected admin
      const onlineCustomersList = Array.from(onlineCustomers);
      socket.emit("customer-status-update", {
        customers: onlineCustomersList,
      });

      // Admin joins ALL conversation rooms
      const conversations = await Conversation.find().select("_id customer");
      conversations.forEach((conv) => {
        socket.join(`conversation-${conv._id}`);
      });

      // Emit admin count to customers
    ...
    socket.on("customer-typing", async (isTyping) => {
      try {
        ...
        adminSockets.forEach((socketId) => {
          ...
          if (adminSocket) {
            adminSocket.emit("customer-typing", {
        ...
        console.log(
          `Customer ${user.name} ${isTyping ? "started" : "stopped"} typing`
        );
      } catch (error) {
    ...
    socket.on("admin-typing", async ({ conversationId, isTyping }) => {
      try {
        ...
        io.to(`conversation-${conversation.customer}`).emit(
          "admin-typing",
          isTyping
        );

        console.log(
          `Admin is ${isTyping ? "typing to" : "stopped typing to"} customer ${
    ...
    socket.on("disconnect", () => {
      console.log(`User disconnected: ${socket.id}, Role: ${socket.role}`);
      ...
      if (socket.role === "admin") {
        ...
        io.emit("admin-online-count", adminSockets.size);
        console.log(`❌ Admin disconnected. Remaining: ${adminSockets.size}`);
      } else if (socket.role === "customer") {
        const wasOnline = onlineCustomers.has(socket.userId);
        onlineCustomers.delete(socket.userId);

        if (wasOnline) {
          console.log(`❌ Customer ${socket.userId} went offline`);
          // IMPORTANT: Broadcast updated customer list when customer goes offline
          broadcastOnlineCustomers();
        }
      }
    });
